...
def show_login():
    ...
    tk.Checkbutton(
        ...
        bg=current_theme["bg"],
        fg=current_theme["fg"],
        selectcolor=current_theme["bg"]).pack(pady=(10, 0))
    ...
    tk.Button(
        ...
        command=lambda: [login_win.destroy(), show_register_window()],
        bg="#FF9800",
        fg="white",
        font=("Segoe UI", 10, "bold"),
        padx=10,
...
def show_register_window():
    ...
    tk.Button(
        reg_win,
        text="Back to Login",
        command=lambda: [reg_win.destroy(), show_login()],
        bg="#757575",
        fg="white",
        font=("Segoe UI", 10, "bold"),
        padx=10,
...
def refresh_transaction_list():
    ...
    try:
        ...
        # Insert updated transactions
         ...
         print(f"Treeview update failed: {e}")
...
def show_transaction_manager():
    ...
    search_var = tk.StringVar(manager_win)
    ...
    tk.Button(
        button_frame,
        text=" View Details",
        command=view_selected_transaction,
        bg="#2196F3",
        fg="white",
    ...
    tk.Button(
        ...
        command=edit_selected_transaction,
        bg="#FF9800",
        fg="white",
        font=("Segoe UI", 10, "bold")).pack(side="left", padx=5)
    ...
    tk.Button(
        button_frame,
        text=" Refresh List",
        command=refresh_transaction_list,
        bg="#4CAF50",
        fg="white",
        font=("Segoe UI", 10, "bold")).pack(side="left", padx=5)
...
def show_edit_transaction_dialog(transaction_id):
    ...
    # Title
    tk.Label(edit_win, text=" Edit Transaction", font=("Segoe UI", 16, "bold"),
    ...
    tk.Label(
        ...
        font=("Segoe UI", 10),
        bg="#f5f5f5",
        fg="#666").grid(row=0, column=1, sticky="w", padx=10, pady=5)
    ...
    tk.Button(
        button_frame,
        text=" Save Changes",
        command=update_transaction_data,
        bg="#4CAF50",
        fg="white",
...
def add_crud_buttons_to_main_window(parent_frame):
    ...
    tk.Button(
        ...
        command=show_transaction_manager,
        bg="#2196F3",
        fg="white",
        font=("Segoe UI", 10, "bold")
...
def show_budget_suggestion():
    ...
    if expense > 0:
        if expense > income:
            suggestion += "• You're spending more than earning!\n"
...
def open_chart_window():
    ...
    tk.Button(
        button_frame,
        text=" Refresh Chart",
        command=lambda: refresh_chart(chart_type, chart_frame),
        bg="#2196F3",
        fg="white",
        font=("Segoe UI", 10, "bold")
    ).pack(side="left", padx=5)
    ...
    tk.Button(
        button_frame,
        text=" Save Chart",
        command=lambda: save_chart(chart_type),
        bg="#4CAF50",
        fg="white",
        font=("Segoe UI", 10, "bold")
    ).pack(side="right", padx=5)
...
def save_chart(chart_type):
    try:
        ...
        if filename:
            messagebox.showinfo(
                "Save Chart",
                f"Chart saving feature will be implemented.\nSelected: {filename}"
            )
    except Exception as e:
...
def show_splash(username):
    ...
    tk.Label(
        splash,
        text="Loading Personal Finance Tracker",
...
def listen_for_command():
    try:
        ...
        update_chat(
            "Sorry, could not understand the audio. Please speak clearly.",
            sender="System"
        )
...
def show_main_app(username):
    ...
    tk.Button(
        header_frame,
        text="Logout",
        command=lambda: [main_window.destroy(), show_login()],
        bg="#F44336",
        fg="white",
        font=("Segoe UI", 10, "bold")
    ...
    entry_frame = tk.LabelFrame(content_frame, text="Add Transaction", font=("Segoe UI", 12, "bold"), 
                            bg="#ECEFF1", fg="#000000", padx=15, pady=15)
    entry_frame.pack(side="left", fill="y", padx=(0, 10))  # left of voice_frame
    ...
    tk.Button(entry_frame, text="Add Transaction", command=add_transactions, bg="#4CAF50", fg="white", 
          font=("Segoe UI", 10, "bold"), padx=20, pady=5).grid(row=3, column=0, columnspan=2, pady=15)
    tk.Button(entry_frame, text="Export to Excel", command=export_to_excel, bg="#28a745", fg="white", 
          font=("Segoe UI", 10, "bold")).grid(row=4, column=0, columnspan=2, pady=5)
    tk.Button(entry_frame, text="Export to PDF", command=export_to_pdf, bg="#dc3545", fg="white", 
          font=("Segoe UI", 10, "bold")).grid(row=5, column=0, columnspan=2, pady=5)


    voice_frame = tk.LabelFrame(content_frame, text="Voice Assistant", font=("Segoe UI", 12, "bold"), 
                            bg="#ECEFF1", padx=15, pady=15)
    voice_frame.pack(side="left", fill="both", expand=True)
    ...
    tk.Button(voice_buttons_frame, text=" Start Listening", command=listen_for_command, bg="#9C27B0", 
          fg="white", font=("Segoe UI", 10, "bold")).pack(side="left", padx=5)
    tk.Button(voice_buttons_frame, text=" Budget Tips", command=show_budget_suggestion, bg="#FF9800", 
          fg="white", font=("Segoe UI", 10, "bold")).pack(side="left", padx=5)

    add_crud_buttons_to_main_window(content_frame)

# Charts section
    chart_frame = tk.LabelFrame(content_frame, text="Financial Charts ", font=("Segoe UI", 12, "bold"), 
                           bg="#ECEFF1", padx=15, pady=15)
...
def main():
    ...
    try:
        ...
        messagebox.showerror("Database Error", 
                           "Failed to initialize database. Please check if the database module is working correctly.")
        return
    ...
    if not os.path.exists(CONFIG_FILE):
        ...
        root = tk.Tk()
        root.withdraw()
        result = messagebox.askyesno("Welcome!", 
