...
from database import create_table, register_user, validate_user, add_transactions as 
db_add_transaction, get_transactions,get_transaction_by_id, update_transaction, 
delete_transaction 
...
def show_login(): 
    ...
    try: 
        ...
        logo_label = tk.Label(login_win, text="💰", font=("Arial", 40), bg=current_theme["bg"], 
fg=current_theme["fg"]) 
        logo_label.pack(pady=10) 
 
    title_label = tk.Label(login_win, text="", font=("Helvetica", 16, "bold"), bg=current_theme["bg"], 
fg=current_theme["fg"]) 
    ...
    tk.Label(login_win, text="Voice-enabled personal finance tracker", font=("Helvetica", 10), 
bg=current_theme["bg"], fg="#AAAAAA").pack(pady=2) 
 
    tk.Label(login_win, text="Username", font=("Segoe UI", 10, "bold"), bg=current_theme["bg"], 
fg=current_theme["fg"]).pack(pady=(20, 5)) 
    username_entry = ttk.Entry(login_win, font=("Segoe UI", 10)) 
    username_entry.pack(pady=5, ipady=3) 
    username_entry.focus() 
 
    tk.Label(login_win, text="Password", font=("Segoe UI", 10, "bold"), bg=current_theme["bg"], 
fg=current_theme["fg"]).pack(pady=5) 
    password_entry = ttk.Entry(login_win, show="*", font=("Segoe UI", 10)) 
    password_entry.pack(pady=5, ipady=3) 
 
    remember_var = tk.BooleanVar() 
    tk.Checkbutton(login_win, text="Remember Me", variable=remember_var, 
bg=current_theme["bg"], fg=current_theme["fg"], selectcolor=current_theme["bg"]).pack(pady=(10, 
0)) 
    ...
    tk.Button(login_win, text="Login", command=attempt_login, bg="#00C853", fg="white", 
activebackground="#00A843", font=("Segoe UI", 10, "bold"), padx=10, pady=5, 
relief="flat").pack(pady=20) 
     
    # Register button 
    tk.Button(login_win, text="Register New Account", command=lambda: [login_win.destroy(), 
show_register_window()], bg="#FF9800", fg="white", font=("Segoe UI", 10, "bold"), padx=10, 
pady=5, relief="flat").pack(pady=5) 
    ...
    tk.Button(login_win, text="Toggle Theme", command=toggle_theme, bg="#455A64", fg="white", 
font=("Segoe UI", 9), relief="flat").pack(pady=5) 
...
def show_register_window(): 
    ...
    try: 
        ...
        logo_label = tk.Label(reg_win, text="💰", font=("Arial", 40), bg=current_theme["bg"], 
fg=current_theme["fg"]) 
        logo_label.pack(pady=10) 
 
    title_label = tk.Label(reg_win, text="", font=("Helvetica", 16, "bold"), bg=current_theme["bg"], 
fg=current_theme["fg"]) 
    title_label.pack(pady=5) 
    ...
    tk.Label(reg_win, text="Voice-enabled personal finance tracker", font=("Helvetica", 10), 
bg=current_theme["bg"], fg="#AAAAAA").pack(pady=2) 
 
    tk.Label(reg_win, text="Username", font=("Segoe UI", 10, "bold"), bg=current_theme["bg"], 
fg=current_theme["fg"]).pack(pady=(20, 5)) 
    username_entry = ttk.Entry(reg_win, font=("Segoe UI", 10)) 
    username_entry.pack(pady=5, ipady=3) 
    username_entry.focus() 
 
    tk.Label(reg_win, text="Password", font=("Segoe UI", 10, "bold"), bg=current_theme["bg"], 
fg=current_theme["fg"]).pack(pady=5) 
...
username.") 
    ...
    reg_btn = tk.Button(reg_win, text="Register", command=register_now, bg="#00C853", fg="white", 
activebackground="#00A843", font=("Segoe UI", 10, "bold"), padx=10, pady=5, relief="flat") 
    reg_btn.pack(pady=20) 
     
    # Back to login Button" 
    tk.Button(reg_win, text="Back to Login", command=lambda: [reg_win.destroy(), show_login()], 
bg="#757575", fg="white", font=("Segoe UI", 10, "bold"), padx=10, pady=5, 
relief="flat").pack(pady=5) 
    ...
    theme_btn = tk.Button(reg_win, text="Toggle Theme", command=toggle_theme, bg="#455A64", 
...
def show_transaction_manager(): 
    ...
    tk.Button(search_frame, text="Search", command=search_transactions, bg="#2196F3", fg="white", font=("Segoe UI", 9, "bold")).pack(side="left", padx=5) 
     
    tk.Button(search_frame, text="Clear", command=lambda: [search_var.set(""), refresh_transaction_list()], bg="#757575", fg="white", font=("Segoe UI", 9, "bold")).pack(side="left", padx=5) 
...
Date: {values[5]} 
    ...
    # Buttons 
    tk.Button(button_frame, text="👁️ View Details", command=view_selected_transaction, bg="#2196F3", fg="white", font=("Segoe UI", 10, "bold")).pack(side="left", padx=5) 
     
    tk.Button(button_frame, text="✏️ Edit Transaction", command=edit_selected_transaction, bg="#FF9800", fg="white", font=("Segoe UI", 10, "bold")).pack(side="left", padx=5) 
...
def show_edit_transaction_dialog(transaction_id): 
    ...
    def update_transaction_data(): 
        try: 
            ...
            update_transaction(transaction_id, new_type, new_amount, new_category, new_note, 
new_date) 
    ...
    tk.Button(button_frame, text="💾 Save Changes", command=update_transaction_data,  
              bg="#4CAF50", fg="white", font=("Segoe UI", 11, "bold"), padx=20).pack(side="left", 
padx=10) 
     
    tk.Button(button_frame, text="❌ Cancel", command=edit_win.destroy,  
              bg="#757575", fg="white", font=("Segoe UI", 11, "bold"), padx=20).pack(side="left", 
padx=10) 
...
{chr(10).join([f"• {cat}: ₹{amt:.2f}" for cat, amt in sorted(summary['income_categories'].items(), 
key=lambda x: x[1], reverse=True)[:3]])} 
 
Top Expense Categories: 
{chr(10).join([f"• {cat}: ₹{amt:.2f}" for cat, amt in sorted(summary['expense_categories'].items(), 
key=lambda x: x[1], reverse=True)[:3]])} 
        """ 
...
def open_chart_window(): 
    ...
    tk.Button(button_frame, text="🔃 Refresh Chart", command=lambda: refresh_chart(chart_type, 
chart_frame),  
              bg="#2196F3", fg="white", font=("Segoe UI", 10, "bold")).pack(side="left", padx=5) 
...
def show_splash(username): 
    ...
    tk.Label(splash, text="Loading Personal Finance Tracker", font=("Helvetica", 14, "bold"), 
bg="#1976D2", fg="white").pack(pady=30) 
 
    progress = ttk.Progressbar(splash, orient="horizontal", mode="determinate", length=300) 
    progress.pack(pady=20) 
 
    status_label = tk.Label(splash, text="Initializing...", font=("Helvetica", 10), bg="#1976D2", 
fg="white") 
...
def create_card(parent, title, variable, color): 
    ...
    value_label = tk.Label(frame, textvariable=variable, bg=color, fg="white", font=("Segoe UI", 12, 
"bold")) 
...
def show_main_app(username): 
    ...
    tk.Label(header_frame, text=f"Welcome back, {username}! 👋🏻", font=("Segoe UI", 18, "bold"), 
fg="white", bg="#37474F").pack(side="left", padx=20, pady=15) 
 
    # Logout button 
    tk.Button(header_frame, text="Logout", command=lambda: [main_window.destroy(), 
show_login()], bg="#F44336", fg="white", font=("Segoe UI", 10, "bold")).pack(side="right", padx=20, 
pady=15) 
    ...
    entry_frame = tk.LabelFrame(content_frame, text="Add Transaction", font=("Segoe UI", 12, 
"bold"), bg="#ECEFF1", fg="#000000", padx=15, pady=15) 
    entry_frame.pack(side="left", fill="y", padx=5) 
 
    tk.Label(entry_frame, text="Type:", font=("Segoe UI", 10, "bold"), bg="#ECEFF1").grid(row=0, 
...
width=15) 
    ...
    tk.Label(entry_frame, text="Amount (₹):", font=("Segoe UI", 10, "bold"), 
bg="#ECEFF1").grid(row=1, column=0, sticky="w", pady=5) 
    amount_entry = ttk.Entry(entry_frame, width=15) 
    amount_entry.grid(row=1, column=1, padx=10, pady=5) 
 
    tk.Label(entry_frame, text="Category:", font=("Segoe UI", 10, "bold"), bg="#ECEFF1").grid(row=2, 
column=0, sticky="w", pady=5) 
    category_entry = ttk.Entry(entry_frame, width=15) 
    category_entry.grid(row=2, column=1, padx=10, pady=5) 
 
    tk.Button(entry_frame, text="Add Transaction", command=add_transactions, bg="#4CAF50", 
fg="white", font=("Segoe UI", 10, "bold"), padx=20, pady=5).grid(row=3, column=0, columnspan=2, 
pady=15) 
    tk.Button(entry_frame, text="Export to Excel", command=export_to_excel, bg="#28a745", 
fg="white", font=("Segoe UI", 10, "bold")).grid(row=4, column=0, columnspan=2, pady=5) 
    tk.Button(entry_frame, text="Export to PDF", command=export_to_pdf, bg="#dc3545", 
fg="white", font=("Segoe UI", 10, "bold")).grid(row=5, column=0, columnspan=2, pady=5) 
 
    # Voice assistant section 
    voice_frame = tk.LabelFrame(content_frame, text="Voice Assistant 🎙️", font=("Segoe UI", 12, 
"bold"), bg="#ECEFF1", padx=15, pady=15) 
    voice_frame.pack(side="left", fill="both", expand=True, padx=5) 
 
    chat_box = tk.Text(voice_frame, height=12, state="disabled", bg="white", wrap="word", 
font=("Segoe UI", 9)) 
    ...
    tk.Button(voice_buttons_frame, text="🎙️ Start Listening", command=listen_for_command, 
bg="#9C27B0", fg="white", font=("Segoe UI", 10, "bold")).pack(side="left", padx=5) 
     
    tk.Button(voice_buttons_frame, text="💡 Budget Tips", command=show_budget_suggestion, 
bg="#FF9800", fg="white", font=("Segoe UI", 10, "bold")).pack(side="left", padx=5) 
 
    add_crud_buttons_to_main_window(content_frame) 
    # Charts section 
    chart_frame = tk.LabelFrame(content_frame, text="Financial Charts 📊", font=("Segoe UI", 12, 
"bold"), bg="#ECEFF1", padx=15, pady=15) 
    chart_frame.pack(side="right", fill="y", padx=5) 
 
    tk.Label(chart_frame, text="Select Chart Type:", font=("Segoe UI", 10, "bold"), 
bg="#ECEFF1").pack(pady=(0, 5)) 
    ...
    preview_label = tk.Label(chart_frame, text="Chart Preview:", font=("Segoe UI", 10, "bold"), 
bg="#ECEFF1") 
    preview_label.pack(pady=(10, 5)) 
     
    preview_frame = tk.Frame(chart_frame, bg="white", relief="sunken", bd=2, width=300, 
height=200) 
...
def main(): 
    ...
    try: 
        ...
        messagebox.showerror("Database Error", "Failed to initialize database. Please check if the 
